...
# Copied and pasted TestOrderServer class above
class TestOrderItemServer(unittest.TestCase):
    """Order Item Server Tests"""
    ...
    def test_get_order_item(self):
        ...
        # get the id of a order
        test_order = self._create_order_items(1)[0] # Need to create the create_order_items method in Issue #12
        resp = self.app.get(
            "/order_items/{}".format(test_order.id), content_type=CONTENT_TYPE_JSON
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        data = resp.get_json()
        self.assertEqual(data["name"], test_order.name)
    ...
    def test_get_order_item_not_found(self):
        """Get a Order Item thats not found"""
        resp = self.app.get("/order_items/0")
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)
